...
sim0()
{
    ...
    for (i = 0; i < ncktin; i++) {
      ...
      /* if a input has changed, schedule the gates connected to it */
        if (sort_wlist[i]->flag0 & CHANGED) {
            sort_wlist[i]->flag0 &= ~CHANGED;
            for (j = 0; j < sort_wlist[i]->nout; j++) {
                if (sort_wlist[i]->onode[j]->nout) {
                    sort_wlist[i]->onode[j]->owire[0]->flag0 |= SCHEDULED;
                }
            }
        }
    } // for every input
     ...
     * it is correct to evaluate the wires in increasing order.   */
    for (i = ncktin; i < ncktwire; i++) {
        if (sort_wlist[i]->flag0 & SCHEDULED) {
            sort_wlist[i]->flag0 &= ~SCHEDULED;
            evaluate0(sort_wlist[i]->inode[0]);
            if (sort_wlist[i]->flag0 & CHANGED) {
                sort_wlist[i]->flag0 &= ~CHANGED;
                for (j = 0; j < sort_wlist[i]->nout; j++) {
                    if (sort_wlist[i]->onode[j]->nout) {
...
nptr n;
{
    unsigned int old_value0, new_value0;
    int i;

    old_value0 = n->owire[0]->value0;
    ...
    switch(n->type) {
        ...
        case NAND:
            new_value0 = 1;
            for (i = 0; i < n->nin; i++) {
                new_value0 = ANDTABLE[n->iwire[i]->value0][new_value0];
            }
            if (n->type == NAND) {
                new_value0 = INV[new_value0];
            }
            break;
        case OR:
        case NOR:
            new_value0 = 0;
            for (i = 0; i < n->nin; i++) {
                new_value0 = ORTABLE[n->iwire[i]->value0][new_value0];
            }
            if (n->type == NOR) {
                new_value0 = INV[new_value0];
            }
            break;
        case NOT:
            new_value0 = INV[n->iwire[0]->value0];
            break;
        case XOR:
            new_value0 = XORTABLE[n->iwire[0]->value0][n->iwire[1]->value0];
            break;
        case EQV:
            new_value0 =INV[(XORTABLE[n->iwire[0]->value0][n->iwire[1]->value0])];
            break;
    }
    if (old_value0 != new_value0) {
        n->owire[0]->flag0 |= CHANGED;
        n->owire[0]->value0 = new_value0;
    }
    return;
}/* end of evaluate */
