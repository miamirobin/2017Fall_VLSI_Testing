    ...
    *num_of_current_detect = 0;
	...
	if ((f->node->type == OUTPUT) ||
	   ...
	   if (tdfsim_only || tdfatpg_only ){
           ...
           // if (!tdfsim_only || (f->activate ==TRUE))  {f->detect = TRUE;}
        }
	else {
	...
	for (w = first_faulty_wire; w; w = wtemp) {
	  ...
	  if (w->flag & OUTPUT) { // if primary output 
        ...
        // then the fault is detected
                        ...
                        if (tdfsim_only || tdfatpg_only) {
                           if (simulated_fault_list[i]->activate ==TRUE)  {
                                 simulated_fault_list[i]->detect = TRUE;
                            } 
                        }
                        else { simulated_fault_list[i]->detect = TRUE;}
    ...
    if (flist) {
	   ...
	   if (f->pnext_undetect->detect == TRUE) { 
               f->pnext_undetect->detected_times++;
               detect_something=1;
               f->pnext_undetect->detect = FALSE;    
               if (doing_podem==1){f->pnext_undetect->patterns=pattern_num;}
           }     
           if (f->pnext_undetect->detected_times==ndet){          
