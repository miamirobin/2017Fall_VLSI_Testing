...
check_test()
{
	...
	//--------------------------------- hole -------------------------------------------
         for (i=0;i<ncktout;i++){                               //check every PO,
            if ( cktout[i]->value==D || cktout[i]->value==B ){  //if any PO has value D or D' ,
                 is_test=TRUE;                                  //return TRUE
                 break;	
	    }
         }
...
fptr fault;
{
	...
	//------------------------------------ hole ----------------------------------------
        int l=1^fault->fault_type ;                 // input a value different with fault type to excite the fault.
        switch (backward_imply(w,l)) {              // use backward_imply function
            case TRUE: pi_is_reach = TRUE; break;   //returns TRUE if we can backtrack to a PI to assign
            case CONFLICT: return(CONFLICT); break; //returns CONFLICT if it is impossible to achieve or set the initial objective
            case FALSE: break;                      //returns FALSE if the backward implication reaches no PI
        }
