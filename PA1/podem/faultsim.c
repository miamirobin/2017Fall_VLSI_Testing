...
{   
    ...
    *num_of_current_detect = 0;
    ...
    for (f = flist; f; f = f->pnext_undetect) {   
      ...
      f->IsDetectedByVector=FALSE; //Sometimes one fault could be detected by one vector more than once, 
                                   // we use IsDetectedByVector to avoid this situation.
	...
	if ((f->node->type == OUTPUT) ||
	    ...
	    if (f->IsDetectedByVector==FALSE){ //If this fault wasn't detected by this vector yet,
                 f->detected_time++;           // then we add detected_time by one,
                 f->IsDetectedByVector=TRUE;   // and we mark this fault as detected by this vector.
            }
            if (f->detected_time==n_detect){   // If the cumulative detected_time of this fault is 
                                               // equal to the value n we entered,
                 f->detect = TRUE;             // then we drop this fault.
            }
	}
	else {
	  ...
	  /* the fault is a gate input fault */
	    ...
	    if (faulty_wire = get_faulty_wire(f,&fault_type)) {
	      ...
	      if (faulty_wire->flag & OUTPUT) {
                 if (f->IsDetectedByVector==FALSE){    //Same
                     f->detected_time++;               //As
                     f->IsDetectedByVector=TRUE;       //Above
                 }
		 if (f->detected_time==n_detect){      //Description
                     f->detect = TRUE;
                 }
	      }
	...
	for (w = first_faulty_wire; w; w = wtemp) {
	  ...
	  //---------------------------------------- hole ---------------------------------------------
	    for (i = 0; i < num_of_fault; i++) {
               if ((w->wire_value2 & Mask[i]) == Unknown[i]){continue;} // If faulty value or good value is equal to unknown(01),
               if ((w->wire_value1 & Mask[i]) == Unknown[i]){continue;} // then we do nothing. 

               if ((w->wire_value1 & Mask[i])!=(w->wire_value2 & Mask[i])){//If faulty value and good value are different,  
                                                                           //then we do the same thing like above. 

                     if (simulated_fault_list[i]->IsDetectedByVector==FALSE){ //If this fault wasn't detected by this vector yet,
                         simulated_fault_list[i]->detected_time++;            //then we add detected_time by one,
                         simulated_fault_list[i]->IsDetectedByVector=TRUE;    //and mark this fault as detected by this vector.
                     }
                     if (simulated_fault_list[i]->detected_time==n_detect){  //If the cumulative detected_time of this fault is 
                         simulated_fault_list[i]->detect = TRUE;             //equal to the value n we entered, 
                     }                                                       //then we drop this fault.
               }
            }
...
{    
	...
	//------------------------------------- hole -------------------------------------------------
        if (fault==STUCK1){                                  //If it is a stuck-at 1 fault,
             faulty_wire->wire_value2 |= Mask[bit_position]; //then the new value is equal to OR(value,11) .
        }
        else if (fault ==STUCK0){                            //If it is a stuck-at 0 fault
             faulty_wire->wire_value2 &= ~Mask[bit_position];//then the new value is equal to AND(value,00) .
        }
    //--------------------------------------------------------------------------------------------
